    ...
    stage ('Build docker') {
      switch ("${env.BRANCH_NAME}") {
        case "master":
          ...
          break
        case "sandbox":
          sh("docker -H :2375 build --build-arg RAILS_ENV=staging --build-arg secretKey=${secretKey} --build-arg FEATURE_DATA_EXPLORER=true -t ${imageTag} .")
          sh("docker -H :2375 build --build-arg RAILS_ENV=staging --build-arg secretKey=${secretKey} --build-arg FEATURE_DATA_EXPLORER=true -t ${dockerUsername}/${appName}:latest .")
          break
        default:
          sh("docker -H :2375 build --build-arg RAILS_ENV=staging --build-arg secretKey=${secretKey} --build-arg FEATURE_DATA_EXPLORER=true -t ${imageTag} .")
          sh("docker -H :2375 build --build-arg RAILS_ENV=staging --build-arg secretKey=${secretKey} --build-arg FEATURE_DATA_EXPLORER=true -t ${dockerUsername}/${appName}:latest .")
