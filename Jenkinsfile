      ...
      switch ("${env.BRANCH_NAME}") {
        case "master":
          sh("docker -H :2375 build
            ...
            --build-arg FEATURE_DATA_EXPLORER=false
            --build-arg FEATURE_DATA_SURVEY=false
            -t ${imageTag} ."
          )
          sh("docker -H :2375 build
            --build-arg RAILS_ENV=production
            ...
            --build-arg FEATURE_DATA_EXPLORER=false
            --build-arg FEATURE_DATA_SURVEY=false
            -t ${dockerUsername}/${appName}:latest ."
          )
          break
        case "sandbox":
          sh("docker -H :2375 build
            ...
            --build-arg USER_SURVEY_SPREADSHEET_URL=https://script.google.com/macros/s/AKfycbzgN1G9IdLYO3KqlTC4gzBxR1UTX5bYXu1qRaiRn1oD9qoaq6s/exec
            -t ${imageTag} ."
          )
          sh("docker -H :2375 build
            --build-arg RAILS_ENV=staging
