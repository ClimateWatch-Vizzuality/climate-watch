  ...
  try {
    ...
    stage ('Build docker') {
      switch ("${env.BRANCH_NAME}") {
        case "master":
          sh("docker -H :2375 build --build-arg RAILS_ENV=production --build-arg secretKey=${secretKey} --build-arg APPSIGNAL_PUSH_API_KEY=${env.CW_APP_SIGNAL} --build-arg FEATURE_DATA_EXPLORER=false --build-arg FEATURE_DATA_SURVEY=false -t ${imageTag} ." )
          sh("docker -H :2375 build --build-arg RAILS_ENV=production --build-arg secretKey=${secretKey} --build-arg APPSIGNAL_PUSH_API_KEY=${env.CW_APP_SIGNAL} --build-arg FEATURE_DATA_EXPLORER=false --build-arg FEATURE_DATA_SURVEY=false -t ${dockerUsername}/${appName}:latest ." )
          break
        case "sandbox":
