...
CREATE SEQUENCE indc_documents_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER SEQUENCE indc_documents_id_seq OWNED BY indc_documents.id;
...
ALTER TABLE ONLY indc_documents ALTER COLUMN id SET DEFAULT nextval('indc_documents_id_seq'::regclass);
...
ALTER TABLE ONLY indc_documents
...
CREATE UNIQUE INDEX index_indc_documents_on_ordering ON indc_documents USING btree (ordering);
...
CREATE UNIQUE INDEX index_indc_documents_on_slug ON indc_documents USING btree (slug);
...
CREATE INDEX index_indc_values_on_document_id ON indc_values USING btree (document_id);
...
ALTER TABLE ONLY indc_values
    ADD CONSTRAINT fk_rails_5b8cda325b FOREIGN KEY (document_id) REFERENCES indc_documents(id);
