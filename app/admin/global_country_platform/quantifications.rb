ActiveAdmin.register_page 'Global Cw Platform Quantifications' do
  section_name = 'quantifications'
  platform_name = 'global_cw_platform'
  s3_folder_path = "#{CW_FILES_PREFIX_TEST}quantifications"
  ...
  content do
    render partial: 'admin/form_upload_datasets', locals: {
      ...
      upload_path: admin_global_cw_platform_quantifications_upload_datafile_path,
      download_path: admin_global_cw_platform_quantifications_download_datafiles_path,
  ...
  page_action :upload_datafile, method: :post do
    ...
    if params[:datafile].content_type != 'text/csv'
      redirect_to admin_global_cw_platform_quantifications_path, alert: "Upload failed
    ...
    redirect_to admin_global_cw_platform_quantifications_path, notice: notice
  end
  ...
  page_action :run_importer, method: :post do
    @job_id = ImportQuantificationsWorker.perform_async
