ActiveAdmin.register_page 'Global Cw Platform Adaptation' do
  ...
  s3_folder_path = "#{CW_FILES_PREFIX_TEST}adaptation"
  ...
  content do
    render partial: 'admin/form_upload_datasets', locals: {
      ...
      download_single_data_file_path: admin_global_cw_platform_adaptation_download_datafile_path,
  ...
  page_action :upload_datafile, method: :post do
    ...
    if params[:datafile].content_type != 'text/csv'
      redirect_to admin_global_cw_platform_adaptation_path, alert: "Upload failed
  ...
  page_action :run_importer, method: :post do
    @job_id = ImportAdaptationWorker.perform_async
