ActiveAdmin.register_page 'Global Cw Platform Timeline' do
  ...
  s3_folder_path = "#{CW_FILES_PREFIX_TEST}timeline"
  ...
  page_action :upload_datafile, method: :post do
    ...
    if params[:datafile].content_type != 'text/csv'
      redirect_to admin_global_cw_platform_timeline_path, alert: "Upload failed
    ...
    redirect_to admin_global_cw_platform_timeline_path, notice: notice
  end
  ...
  page_action :run_importer, method: :post do
    @job_id = ImportTimelineWorker.perform_async
