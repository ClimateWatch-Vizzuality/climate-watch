ActiveAdmin.register_page 'Global Cw Platform Agriculture Emissions' do
  include DataUploader::SharedAdmin

  section_name = 'agriculture_emissions'
  ...
  controller do
    ...
    def s3_folder_path
      "#{CW_FILES_PREFIX}agriculture_emissions"
    end

    def path
      admin_global_cw_platform_agriculture_emissions_path
    end
    ...
    def import_worker
      DataUploader::BaseImportWorker.perform_async(section.id, 'ImportAgricultureEmissions', current_admin_user.email)
    end
  ...
  content do
    render partial: 'data_uploader/admin/form_upload_datasets', locals: {
      ...
      upload_path: admin_global_cw_platform_agriculture_emissions_upload_datafile_path,
      download_path: admin_global_cw_platform_agriculture_emissions_download_datafiles_path,
